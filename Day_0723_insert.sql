...
-- 1) 데이터베이스 "dima4"
-- 2) dima4로 이동
CREATE DATABASE dima4;
USE dima4;

-- 테이블 생성
-- 일련번호(int), 이름(varchar(15)), 주소(100), 나이(decimal(3)) 
DROP TABLE sample;
...
-- [연습] fitness 회원 정보를 저장할 수 있는 테이블을 생성하시오
...
(
	...
	, name varchar(20) NOT NULL -- NOT NULL은 반드시 컬럼 레벨로
	, gender char(1) 
	, height decimal(5, 2) 
	, weight decimal(5, 2)
...
SELECT 
TABLE_NAME, COLUMN_NAME, IS_NULLABLE, COLUMN_KEY 
FROM information_schema.COLUMNS
WHERE TABLE_SCHEMA = 'dima4'
AND table_name = 'fitness';

-- 계산을 해서 데이터를 넣을 경우 소실되는 자릿수가 발생할 수 있다는
-- 경고가 발생한다.
INSERT INTO fitness
...
(name, gender, height, weight, bmi)
VALUES
('김남주', '여', 168, 50, weight / (height * height * 0.0001));
